...
from ..components import (
    common,
    get_kana_training_job_spec,
)


@dsl.pipeline(name="ainu_kana_pipeline", pipeline_root="ainu-lm")
def ainu_kana_pipeline(
    project_id: str,
    location: str,
    service_account: str,
    tensorboard_id: str,
    ...
    get_kana_training_job_spec_op = get_kana_training_job_spec(
        train_image_uri=train_image_uri,
        dataset_name=hf_dataset_repo,
        dataset_revision=get_dataset_revision_op.output,
        hub_model_id="aynumosir/mt5-base-ainu-kana",
        push_to_hub=True,
    ).set_display_name("カナ変換ジョブの仕様を取得")

    (
        CustomTrainingJobOp(
            project=project_id,
            display_name=f"ainu-lm-kana-{training_job_suffix}",
            base_output_directory=f"{get_base_output_directory_op.output}",
            worker_pool_specs=get_kana_training_job_spec_op.output,
            location=location,
            tensorboard=f"projects/{project_id}/locations/{location}/tensorboards/{tensorboard_id}",
            service_account=service_account,
        )
        .set_display_name("カナ変換の訓練")
